...
- [修复] 通知渲染层对 `intelligence.risk_alerts` / `intelligence.positive_catalysts` / `intelligence.latest_news` 新增 `_coerce_str_list` 防御：DSA prompt schema 要求 LLM 将这些字段返回为 JSON 数组，但 LLM 偶尔会返回单个字符串（如 `"数据缺失，无法判断"`）或 `None`，原先直接 `for item in value` 会把字符串按字符拆分，每个字符渲染成独立 bullet，产生 `- 数 / - 据 / - 缺 / - 失` 的乱码（2026-04-10 14:26 UTC 生产 cron run 实际观测到 16 条单字符 bullet）。修复：在 `src/notification.py` 顶部新增 `_coerce_str_list(value, max_items=None)` 辅助，规则是 `None` / 空串 → `[]`，非空字符串 → `[value]`（整段作为一条 bullet，不按字符拆），list/tuple → 过滤 `None` 与纯空白项后的字符串列表，其它标量 → `[str(value)]`。在 3 处渲染点统一接入：(1) `generate_dashboard_report` 决策仪表盘 / DSA 简报里的 `risk_alerts` / `positive_catalysts` / `latest_news`（`latest_news` 为 list 时用 `" / "` 合并成单行，避免 f-string 渲染 Python repr）；(2) 单股紧凑报告里带 50 字符截断和最多 2 条的渲染；(3) 移动端汇总里带 60 字符截断和最多 3 条的渲染，`max_items` 现在在过滤空白项之后才裁剪，保证上限语义是有效条数而非原始 input 长度。新增 `tests/test_notification.py::TestCoerceStrList`（13 用例）锁定契约：`None` / 空串 / 空 list → `[]`，list of strings 透传，**非空字符串绝不按字符拆分**，list 中 `None` / 空白项被过滤，dict / int / tuple 等其它类型的兜底行为，`max_items` 在过滤后应用，`0` 仍渲染为 `"0"` 而不是被误判为空。
- [修复] 趋势分析读取 `stock_daily` 历史时改走新的 `DatabaseManager.get_source_consistent_tail(code, start, end)` 方法，只保留与最新一行 `data_source` 相同的**末尾连续切片**，彻底防止 `CCXTCryptoFetcher`（单交易所 base-currency 归一化后的 USD notional，例如 Kraken 约占 BTC 全球份额 0.37%）与 `YfinanceFetcher`（全球聚合 USD notional，量级约大 270×）在 rolling 窗口内混合，导致 `src/stock_analyzer.py::_analyze_volume` 计算 `volume_ratio_5d` 时产生 ~270× 或 ~1/270× 的虚假暴量 / 缩量信号，进而误触发 `VOLUME_HEAVY_RATIO` / `VOLUME_SHRINK_RATIO` 状态并污染 LLM prompt 的量能描述。修复点：`src/core/pipeline.py::analyze_stock` 与 `src/agent/tools/analysis_tools.py::_fetch_trend_data` 两处调用点统一切换到新 helper；`stock_daily.to_dict` 已包含 `data_source` 字段无需 schema 变更；当源在窗口尾部切换导致同源切片 < 5 行时，下游 `volume_ratio_5d` 降级为默认值（宁愿让 LLM 看到"数据不足"也不让 LLM 看到"假 270× 暴量"）。新增 `tests/test_storage.py::TestSourceConsistentTail`（5 用例）覆盖空窗口、全窗口同源、尾部源切换裁剪、跨源 rolling 比例爆炸回归守卫、单源股票无副作用。
- [修复] `CCXTCryptoFetcher` 输出的 volume 字段此前直接透传 ccxt.fetch_ohlcv 返回的 **base currency** 单位（例如 BTC/USD 返回的是 BTC 数量），与 `YfinanceFetcher` 返回的 **USD notional**（全球聚合成交额）存在约 6 个数量级的单位差异。当 crypto 路由在 `CCXTCryptoFetcher` 首选和 `YfinanceFetcher` 兜底之间切换时，同一张 `stock_daily` 表中 volume 字段会出现 base/notional 混合，下游 `signals.volume.volume_ratio_5d` / `volume_anomaly` 会产生虚假的 spike/shrink 异常。修复：在 `get_daily_data` 出口处将 daily volume 归一化为 USD notional 代理值，**整批用同一个常量乘子** (`df.close.iloc[-2]`，即 T-1 收盘价) 而非每行各自的 close。理由：(1) `data_provider.base.DataFetcherManager._calculate_indicators` 在 fetcher 输出上 rolling 计算 `volume_ratio = today_volume / mean(prev5_volume)`，若每行用各自 close 会引入 `today_close / weighted_mean_prev5_close` 的 bias 因子，BTC 2-3% 日波动下 5d 累积可达 ±5-10%；常量缩放在 ratio 中完全相互抵消，**zero drift**。(2) 用 T-1 收盘价（已收盘、不变）而非 `iloc[-1]`（intraday-partial、随时间变化），cron intraday 多次 dispatch 同一日期的归一化值完全稳定，DB upsert 行为确定。`get_realtime_data` 优先读取 ccxt ticker 自带的 `quoteVolume`，缺失时回退到 `baseVolume * last`。Kraken 仍是单交易所，归一化后的绝对值约为全球 notional 的 0.37%，rolling 窗口内的相对比例与原始 base 完全等价（live 对比 BTC/ETH 的 CCXT/YF 比例一致落在 0.0037-0.0039 区间）。新增 `tests/test_ccxt_crypto_fetcher.py::CCXTVolumeNormalizationTest`（10 用例）覆盖 daily 归一化公式、滚动窗口不变性、**趋势 close 下 ratio 漂移精确为零**、**intraday 多次 dispatch 稳定性**、单行批次降级、空响应、非 crypto 代码、realtime quoteVolume 优先 / baseVolume×last 回退 / 双字段缺失容错。
- [测试] 修复 `test_notification.test_generate_dashboard_report_localizes_english_fallback` 在 DSA_MODE 默认开启后失败的问题：DSA_MODE=true 时 `generate_dashboard_report` 渲染为中文"数据信号简报"格式并抑制 legacy "Decision Dashboard / Action Levels" 章节，导致英文本地化断言失效。修复：用 `mock.patch.dict(os.environ, {"DSA_MODE": "false"})` 包裹被测调用以 opt-out DSA 模式，继续覆盖 legacy 英文决策仪表盘的回退渲染路径。
