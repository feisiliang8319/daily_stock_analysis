...
- [修复] `CCXTCryptoFetcher` 输出的 volume 字段此前直接透传 ccxt.fetch_ohlcv 返回的 **base currency** 单位（例如 BTC/USD 返回的是 BTC 数量），与 `YfinanceFetcher` 返回的 **USD notional**（全球聚合成交额）存在约 6 个数量级的单位差异。当 crypto 路由在 `CCXTCryptoFetcher` 首选和 `YfinanceFetcher` 兜底之间切换时，同一张 `stock_daily` 表中 volume 字段会出现 base/notional 混合，下游 `signals.volume.volume_ratio_5d` / `volume_anomaly` 会产生虚假的 spike/shrink 异常。修复：在 `get_daily_data` 出口处将 daily volume 归一化为 USD notional 代理值（`base_volume * close`，round 2 位）；`get_realtime_data` 优先读取 ccxt ticker 自带的 `quoteVolume`，缺失时回退到 `baseVolume * last`。Kraken 仍是单交易所，归一化后的绝对值约为全球 notional 的 0.3-0.4%，但 rolling 窗口内的相对比例得到保留（live 对比 BTC/ETH 的 CCXT/YF 比例一致落在 0.0036 附近）。新增 `tests/test_ccxt_crypto_fetcher.py::CCXTVolumeNormalizationTest`（7 用例）覆盖 daily 归一化公式、滚动窗口不变性、空响应、非 crypto 代码、realtime quoteVolume 优先 / baseVolume×last 回退 / 双字段缺失容错。
